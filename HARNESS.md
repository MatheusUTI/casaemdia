...
|---|--------------------------|-------------------------|-----------|
| **1** | A alteração planejada viola alguma diretriz escrita no **`AI_RULES.md`**? | O `AI_RULES.md` é a constituição do aplicativo. Ele delimita o que a aplicação é, o posicionamento do produto, e as regras rígidas anti-alucinação. | **[SIM / NÃO]** |
| **2** | A alteração planejada viola algum dos fluxos de dados ou padrões definidos no **`SDD.md`**? | O SDD rege o fluxo unilinear obrigatório de dados (`UI` ↔ `ViewModel` ↔ `Repository` ↔ `Mappers` ↔ `Room`). Alterações que deitem as premissas deste fluxo corrompem a manutenibilidade. | **[SIM / NÃO]** |
...
| **12** | O plano cria tabelas no Room ou armazena arquivos **fora das 4 entidades arquiteturais previstas** no escopo principal do MVP? | O escopo restringe as tabelas fundamentais em: `AssetEntity`, `ControlItemEntity`, `AttachmentEntity` e `HistoryEntryEntity`. Modificações estruturais exigem alinhamento. | **[SIM / NÃO]** |
...
> 3. Se a modificação for de fato justificada por alteração de requisitos do negócio, deve-se primeiro atualizar os documentos de governança (`AI_RULES.md` e `SDD.md`) recebendo anuência do engenheiro principal antes de alterar uma única linha de código fonte.
